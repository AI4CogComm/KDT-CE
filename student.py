...
def load_data(mat_path, num_samples, Nx, Ny, snr_db=10):
...
if __name__ == "__main__":
    ...
    teacher_model = tf.keras.models.load_model(r"D:\LISHUANGSHUANG\ssl_kd_mat\MAT\1_MATCENet_-5dB_200ep_e3lr.hdf5")
    teacher_model.trainable = False

    H_in, H_out = load_data('f1n5_256ANTS_1000by100.mat', 100000, Nx, Ny, snr)
    val_size = int(val_split * len(H_in))
    train_x, val_x = H_in[:-val_size], H_in[-val_size:]
    train_y, val_y = H_out[:-val_size], H_out[-val_size:]
    # 假设你读取的是一个 .npz 文件
    teacher_y = load_teacher_predictions(r"D:\LISHUANGSHUANG\ssl_kd_mat\MAT\1_teacher_pred_result_-5dB_200ep_e3lr.npy")
    ...
    model_save_path = "tukd5_0.5_0.5_2_-5dB.hdf5"
    ...
    H_in_test, H_out_test = load_data('f1n5_256ANTS_10by200.mat', 2000, Nx, Ny, snr)
